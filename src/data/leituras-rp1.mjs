/** Leituras do lugar de direção com os acertos auditados em acertos-rp1.json. */
export const LEITURAS_RP1 = {
  "ipc-variacao-homologa": {
    "pt": [
      {
        "sinal": {
          "positivo": [
            "Em ",
            {
              "periodo": "proprio"
            },
            " ",
            "os preços no consumidor estavam, em média,",
            " ",
            {
              "claim": "proprio",
              "sufixo": " %"
            },
            " acima dos de há um ano."
          ],
          "negativo": [
            "Em ",
            {
              "periodo": "proprio"
            },
            " ",
            "os preços no consumidor estavam, em média,",
            " ",
            {
              "claim": "proprio",
              "sufixo": " %"
            },
            " face aos de há um ano, ou seja, abaixo deles."
          ],
          "zero": [
            "Em ",
            {
              "periodo": "proprio"
            },
            " ",
            "os preços no consumidor estavam, em média,",
            " ao mesmo nível de há um ano."
          ]
        }
      },
      " É a inflação: o INE mede-a num cabaz de bens e serviços que representa o que as famílias compram.",
      {
        "compara": "anterior",
        "maior": [
          " ",
          "A variação é maior do que a do mês anterior."
        ],
        "menor": [
          " ",
          "A variação é menor do que a do mês anterior."
        ],
        "igual": [
          " ",
          "A variação é igual à do mês anterior."
        ]
      }
    ],
    "en": [
      {
        "sinal": {
          "positivo": [
            "In ",
            {
              "periodo": "proprio"
            },
            " ",
            "consumer prices were, on average,",
            " ",
            {
              "claim": "proprio",
              "sufixo": " %"
            },
            " above a year earlier."
          ],
          "negativo": [
            "In ",
            {
              "periodo": "proprio"
            },
            " ",
            "consumer prices were, on average,",
            " ",
            {
              "claim": "proprio",
              "sufixo": " %"
            },
            " compared with a year earlier, that is, below."
          ],
          "zero": [
            "In ",
            {
              "periodo": "proprio"
            },
            " ",
            "consumer prices were, on average,",
            " at the same level as a year earlier."
          ]
        }
      },
      " That is inflation: the INE measures it on a basket of goods and services that represents what households buy.",
      {
        "compara": "anterior",
        "maior": [
          " ",
          "The change is larger than the previous month’s."
        ],
        "menor": [
          " ",
          "The change is smaller than the previous month’s."
        ],
        "igual": [
          " ",
          "The change is the same as the previous month’s."
        ]
      }
    ]
  },
  "ipc-variacao-media-12-meses": {
    "pt": [
      "Na média dos doze meses até ",
      {
        "periodo": "proprio"
      },
      ", os preços no consumidor variaram ",
      {
        "claim": "proprio",
        "sufixo": " %"
      },
      " face aos doze meses anteriores: é a inflação média de um ano, que amortece as subidas e descidas de cada mês.",
      {
        "compara": "anterior",
        "maior": [
          " ",
          "A variação é maior do que a do mês anterior."
        ],
        "menor": [
          " ",
          "A variação é menor do que a do mês anterior."
        ],
        "igual": [
          " ",
          "A variação é igual à do mês anterior."
        ]
      }
    ],
    "en": [
      "On average over the twelve months to ",
      {
        "periodo": "proprio"
      },
      ", consumer prices changed by ",
      {
        "claim": "proprio",
        "sufixo": " %"
      },
      " compared with the previous twelve months: that is the average inflation of a year, which smooths out each month’s rises and falls.",
      {
        "compara": "anterior",
        "maior": [
          " ",
          "The change is larger than the previous month’s."
        ],
        "menor": [
          " ",
          "The change is smaller than the previous month’s."
        ],
        "igual": [
          " ",
          "The change is the same as the previous month’s."
        ]
      }
    ]
  },
  "ipc-alimentacao-variacao-homologa": {
    "pt": [
      {
        "sinal": {
          "positivo": [
            "Em ",
            {
              "periodo": "proprio"
            },
            " ",
            "os preços dos alimentos e das bebidas não alcoólicas estavam",
            " ",
            {
              "claim": "proprio",
              "sufixo": " %"
            },
            " acima dos de há um ano."
          ],
          "negativo": [
            "Em ",
            {
              "periodo": "proprio"
            },
            " ",
            "os preços dos alimentos e das bebidas não alcoólicas estavam",
            " ",
            {
              "claim": "proprio",
              "sufixo": " %"
            },
            " face aos de há um ano, ou seja, abaixo deles."
          ],
          "zero": [
            "Em ",
            {
              "periodo": "proprio"
            },
            " ",
            "os preços dos alimentos e das bebidas não alcoólicas estavam",
            " ao mesmo nível de há um ano."
          ]
        }
      },
      {
        "compara": "anterior",
        "maior": [
          " ",
          "A variação é maior do que a do mês anterior."
        ],
        "menor": [
          " ",
          "A variação é menor do que a do mês anterior."
        ],
        "igual": [
          " ",
          "A variação é igual à do mês anterior."
        ]
      }
    ],
    "en": [
      {
        "sinal": {
          "positivo": [
            "In ",
            {
              "periodo": "proprio"
            },
            " ",
            "the prices of food and non-alcoholic beverages were",
            " ",
            {
              "claim": "proprio",
              "sufixo": " %"
            },
            " above a year earlier."
          ],
          "negativo": [
            "In ",
            {
              "periodo": "proprio"
            },
            " ",
            "the prices of food and non-alcoholic beverages were",
            " ",
            {
              "claim": "proprio",
              "sufixo": " %"
            },
            " compared with a year earlier, that is, below."
          ],
          "zero": [
            "In ",
            {
              "periodo": "proprio"
            },
            " ",
            "the prices of food and non-alcoholic beverages were",
            " at the same level as a year earlier."
          ]
        }
      },
      {
        "compara": "anterior",
        "maior": [
          " ",
          "The change is larger than the previous month’s."
        ],
        "menor": [
          " ",
          "The change is smaller than the previous month’s."
        ],
        "igual": [
          " ",
          "The change is the same as the previous month’s."
        ]
      }
    ]
  },
  "ipc-sem-habitacao-variacao-media-12-meses": {
    "pt": [
      "Na média dos doze meses até ",
      {
        "periodo": "proprio"
      },
      ", os preços no consumidor sem a habitação variaram ",
      {
        "claim": "proprio",
        "sufixo": " %"
      },
      " face aos doze meses anteriores. O valor de agosto serve de referência para a atualização das rendas no ano seguinte.",
      {
        "compara": "anterior",
        "maior": [
          " ",
          "A variação é maior do que a do mês anterior."
        ],
        "menor": [
          " ",
          "A variação é menor do que a do mês anterior."
        ],
        "igual": [
          " ",
          "A variação é igual à do mês anterior."
        ]
      }
    ],
    "en": [
      "On average over the twelve months to ",
      {
        "periodo": "proprio"
      },
      ", consumer prices excluding housing changed by ",
      {
        "claim": "proprio",
        "sufixo": " %"
      },
      " compared with the previous twelve months. The August value is the reference figure for updating rents in the following year.",
      {
        "compara": "anterior",
        "maior": [
          " ",
          "The change is larger than the previous month’s."
        ],
        "menor": [
          " ",
          "The change is smaller than the previous month’s."
        ],
        "igual": [
          " ",
          "The change is the same as the previous month’s."
        ]
      }
    ]
  },
  "remuneracao-bruta-mensal-media": {
    "pt": [
      "No ",
      {
        "periodo": "proprio"
      },
      ", quem trabalha por conta de outrem ganhou em média ",
      {
        "claim": "proprio",
        "sufixo": " euros por mês"
      },
      ", antes de descontos e contando os subsídios, nos postos de trabalho declarados à Segurança Social e à Caixa Geral de Aposentações; cada pessoa conta tantas vezes quantos os empregos que tem.",
      {
        "compara": "anterior",
        "maior": [
          " ",
          "Mais do que no mesmo trimestre de há um ano."
        ],
        "menor": [
          " ",
          "Menos do que no mesmo trimestre de há um ano."
        ],
        "igual": [
          " ",
          "O mesmo que no mesmo trimestre de há um ano."
        ]
      }
    ],
    "en": [
      "In the ",
      {
        "periodo": "proprio"
      },
      ", employees earned on average ",
      {
        "claim": "proprio",
        "sufixo": " euros a month"
      },
      ", before deductions and including holiday and Christmas pay, in the jobs declared to Social Security and to the civil-service pension fund; each person counts as many times as the jobs they hold.",
      {
        "compara": "anterior",
        "maior": [
          " ",
          "More than in the same quarter a year earlier."
        ],
        "menor": [
          " ",
          "Less than in the same quarter a year earlier."
        ],
        "igual": [
          " ",
          "The same as in the same quarter a year earlier."
        ]
      }
    ]
  },
  "pensao-media-anual-2025": {
    "pt": [
      "Em ",
      {
        "periodo": "proprio"
      },
      " o valor das pensões pagas pela Segurança Social foi, em média, de ",
      {
        "claim": "proprio"
      },
      " euros por pensionista no ano inteiro, contando as pensões pagas de velhice, de invalidez e de sobrevivência.",
      {
        "compara": "anterior",
        "maior": [
          " ",
          "Subiu face a",
          " ",
          {
            "periodo": "anterior"
          },
          "."
        ],
        "menor": [
          " ",
          "Desceu face a",
          " ",
          {
            "periodo": "anterior"
          },
          "."
        ],
        "igual": [
          " ",
          "Ficou igual a",
          " ",
          {
            "periodo": "anterior"
          },
          "."
        ]
      }
    ],
    "en": [
      "In ",
      {
        "periodo": "proprio"
      },
      " the amount of pensions paid by Social Security was, on average, ",
      {
        "claim": "proprio"
      },
      " euros per pensioner over the whole year, counting old-age, invalidity and survivors’ pensions paid.",
      {
        "compara": "anterior",
        "maior": [
          " ",
          "Up from",
          " ",
          {
            "periodo": "anterior"
          },
          "."
        ],
        "menor": [
          " ",
          "Down from",
          " ",
          {
            "periodo": "anterior"
          },
          "."
        ],
        "igual": [
          " ",
          "Unchanged from",
          " ",
          {
            "periodo": "anterior"
          },
          "."
        ]
      }
    ]
  },
  "beneficiarios-do-rsi-por-mil-2024": {
    "pt": [
      "Em ",
      {
        "periodo": "proprio"
      },
      ", por cada mil pessoas em idade ativa, ",
      {
        "claim": "proprio"
      },
      " recebiam o rendimento social de inserção, um apoio da Segurança Social para satisfazer necessidades essenciais e favorecer a inserção laboral, social e comunitária.",
      {
        "compara": "anterior",
        "maior": [
          " ",
          "Mais do que em",
          " ",
          {
            "periodo": "anterior"
          },
          "."
        ],
        "menor": [
          " ",
          "Menos do que em",
          " ",
          {
            "periodo": "anterior"
          },
          "."
        ],
        "igual": [
          " ",
          "O mesmo que em",
          " ",
          {
            "periodo": "anterior"
          },
          "."
        ]
      }
    ],
    "en": [
      "In ",
      {
        "periodo": "proprio"
      },
      ", for every thousand people of working age, ",
      {
        "claim": "proprio"
      },
      " received social insertion income, a Social Security benefit to meet essential needs and support integration into employment, society and the community.",
      {
        "compara": "anterior",
        "maior": [
          " ",
          "More than in",
          " ",
          {
            "periodo": "anterior"
          },
          "."
        ],
        "menor": [
          " ",
          "Fewer than in",
          " ",
          {
            "periodo": "anterior"
          },
          "."
        ],
        "igual": [
          " ",
          "The same as in",
          " ",
          {
            "periodo": "anterior"
          },
          "."
        ]
      }
    ]
  },
  "linha-de-risco-de-pobreza-2025": {
    "pt": [
      "No inquérito de ",
      {
        "periodo": "proprio"
      },
      ", uma pessoa que vivesse sozinha estava em risco de pobreza se tivesse menos de ",
      {
        "claim": "proprio"
      },
      " euros por ano para viver no ano anterior, depois dos impostos e das contribuições sociais e contando as prestações sociais: é a linha que o Eurostat traça a ",
      {
        "nl": "60",
        "motivo": "escala-de-instrumento"
      },
      " % do rendimento mediano do país, ajustado ao tamanho e à composição de cada família: é o do meio, em que metade da população tem mais e metade tem menos.",
      {
        "compara": "anterior",
        "maior": [
          " ",
          "Subiu face a",
          " ",
          {
            "periodo": "anterior"
          },
          "."
        ],
        "menor": [
          " ",
          "Desceu face a",
          " ",
          {
            "periodo": "anterior"
          },
          "."
        ],
        "igual": [
          " ",
          "Ficou igual a",
          " ",
          {
            "periodo": "anterior"
          },
          "."
        ]
      }
    ],
    "en": [
      "In the survey for ",
      {
        "periodo": "proprio"
      },
      ", a person living alone was at risk of poverty with less than ",
      {
        "claim": "proprio"
      },
      " euros a year to live on in the previous year, after taxes and social contributions and including social benefits: that is the line Eurostat draws at ",
      {
        "nl": "60",
        "motivo": "escala-de-instrumento"
      },
      " % of the country’s median income, adjusted for each household’s size and composition: the one in the middle, where half the population has more and half has less.",
      {
        "compara": "anterior",
        "maior": [
          " ",
          "Up from",
          " ",
          {
            "periodo": "anterior"
          },
          "."
        ],
        "menor": [
          " ",
          "Down from",
          " ",
          {
            "periodo": "anterior"
          },
          "."
        ],
        "igual": [
          " ",
          "Unchanged from",
          " ",
          {
            "periodo": "anterior"
          },
          "."
        ]
      }
    ]
  }
};
