...
export function linhasAtrasadas() {
  return [...loadClaims().values()].filter((linha) => atrasoDaLinha(linha) !== null);
}

/**
 * Quantas séries e quantas linhas estão atrasadas.
 *
...
export function contagensDoAtraso() {
  const linhas = linhasAtrasadas();
  const ids = new Set(linhas.map((l) => atrasoDaLinha(l)?.id).filter((x) => typeof x === 'string'));
  return { series: ids.size, linhas: linhas.length };
}
