# O Estado do País · the structure and the vocabulary (17.09.2026)

*Written by the direction seat (Claude Fable 5.1) on 17.09.2026 at Nuno's request, after his reading of the live site on 16.09 and 17.09: pages that explain themselves, several doors to the same thing, two names for the same subject, studies scattered from the places they are about, words translated from English. This decides the surface of the site: what each page is for, how the pages connect, and which words the site uses. It does not touch the foundations: the ledger with every number traced to its source, the receipts, the checks, the motor, the studies' ledgers, the data of the 308 municipalities, the map. Facts about the site as it is today are marked [F]; the seat's proposals [P]; what Nuno decides [D].*

## 0 · What is wrong today, in five lines

[F] The menu has thirteen entries: Início, Concelhos, Estudos, Números e fontes, Regiões, Distritos, Domínios, Áreas de governo, Portugal na União Europeia, Sobre, Método, Correções, Agenda. [F] The measures live under nine "áreas de governo" (the ministries' names) while the front page lists eighteen "domínios" with counts; a reader meets two taxonomies for one thing. [F] The studies have their own island: a study page with an "Edições" box, two doors ("Ler no sítio", "Ler o documento"), and a list grouped by subject; the Évora page lists them by hand and missed the newest. [F] Nearly every page opens with a sentence that explains the page or the project, and source credits repeat under the map, under the cards, in the text. [F] Several words are English words in Portuguese clothes ("sítio" for the website, "casa" for the project).

## 1 · The line: one grammar for every place [P]

Portugal is read along one line: **the country, the regions (7), the districts and islands (29), the municipalities (308)**. Every level is a page with the same grammar, in this order, and nothing else:

*Correction by the seat, 21.09.2026, measured on the official map the project hosts (CAOP 2025, `public/dados/caop-2025-municipios-*.csv`): the regions are **nine** (the NUTS II of 2024, the nine the site already has), not seven, and the line is not a strict nesting: six districts (Aveiro, Guarda, Leiria, Lisboa, Setúbal, Viseu; 102 of the 308 municipalities) have municipalities in two regions. So a municipality's line states its own region and its own district, each read from the map; the places page lists the nine regions and the 29 districts and islands as two plain lists; a region shows its municipalities (grouped by district) and a district says which regions its municipalities are in (B3). Where §1.6 and §8 say "a region lists its districts", read this instead. The detail is in the B1 brief, §6.*

1. **The name, and its place in the line.** «Évora», and above it, small: Portugal › Alentejo › Évora (district) › Évora. Each part is a link.
2. **The reading.** What the numbers of this place say now, in a few sentences written by the project, each with its number and its source behind it. For the country this is the sentence that is on the front page today (the debt, the unemployment). For a municipality it is one to three sentences. It is the first thing on the page, and it is content, not explanation.
3. **The numbers, by subject.** The eighteen subjects (see §2), each with its measures for this place, as cards: the name, the value with its unit and period, one sentence, the ruler with the previous period and the reference. Subjects without a measure for this place are not shown. No "13 medidas" counts, no grey cells: the grid fills, and the last row is as wide as its cards.
4. **The studies about this place.** Each with its title and its reading (the opening's «Em resumo», first paragraph), newest first. One link: the study.
5. **What changed.** Dated lines: a new value, a new study, a correction. Newest first.
6. **Where to go from here.** The places inside this one (a district lists its municipalities; a region its districts), and nothing else.

The front page is the country's page with the map as the door to the places: the map, then the reading, then the numbers by subject, then the latest studies, then what changed. No subtitle under the name. No credit under the map (it lives on the Método page, once, as the licence requires).

## 2 · The subjects: one axis, one word [P]

The eighteen domains of the content charter are the only subject axis. The nine "áreas de governo" disappear as pages; which ministry publishes a measure is a fact of the measure's receipt, not a place to browse. For the reader the word is **«Temas»** (the eighteen themes), because that is the word a Portuguese newspaper uses for its sections; "domínio" stays a word of the charter and the code. Each theme has one page: the reading for the theme, its measures for the country, and the places where the theme has numbers. Portugal in the European Union is a theme's view, not a separate page family: the European comparison lives inside each measure (the ruler already shows it) and the "Portugal na União Europeia" page becomes the theme page's European section.

## 3 · The studies: one door, attached to their place [P]

A study is a page with: the title; the project's reading (the opening: «Em resumo», «O que este projeto conclui», «O que podia funcionar melhor»); the text of the study; at the end, one line: «Documento original (PDF)» for the edition as published, and the date. There is no "Edições" box and no second door. The English edition is reached by the site's language switch, as every other page. The studies list shows every study newest first, with its title, its place and theme, its date and the first paragraph of its reading. The front page and each place page show their studies the same way. A study's numbers keep their receipts exactly as today.

## 4 · The menu: five entries [P]

**Portugal · Lugares · Temas · Estudos · Sobre.** "Portugal" is the country's page (the front page). "Lugares" opens the line: the map, the regions, the districts, the search for a municipality. "Temas" opens the eighteen. "Estudos" the list. "Sobre" holds what the site says about itself, once: who makes it and how (today's Sobre and Método, joined), the corrections and the right of reply, the agenda of the next official releases. Método, Correções and Agenda stay as pages, reached from Sobre and from the footer. "Números e fontes" (the ledger index) stays reachable from every receipt and from the footer; it is not a menu entry.

## 5 · The three rules that the checks enforce [P]

1. **Nothing explains itself.** On a reader page every sentence is content: a number, a name, a fact, a reading. Section labels come from a closed list («Temas», «Estudos sobre este lugar», «O que mudou», «Fonte», «Verificado a»). A sentence that describes the page, the project, the method or the source belongs on Sobre, Método or the receipt, once. The build fails on any other sentence.
2. **One door per thing.** One link to a study, one to a place, one to a theme. No box that lists doors. No link whose text explains what it opens.
3. **The vocabulary is the site's, in European Portuguese, and use is checked in context.** Not a list of banned words: a list of things and the word for each, with where the word belongs (§6). "Casa" is housing; the project is «O Estado do País» or «este projeto». "Sítio" is a place; a web page is «página» and the whole is «o projeto». The check flags a word used outside its place, not the word.

## 6 · The vocabulary [P, for Nuno to correct]

| the thing | the word on the site | where it belongs, and where not |
|---|---|---|
| the project | O Estado do País; este projeto | never «a casa», never «o sítio», never «o site» |
| a page of the site | página | never «sítio» |
| the whole site, when it must be named | o projeto; este endereço only in technical notes | never «o sítio» |
| a subject | tema | never «domínio» or «área de governo» on a reader page; «domínio» in the charter and the code |
| a place | lugar; concelho, distrito, ilha, região, país | never «unidade» on a reader page |
| a measured quantity | medida | «indicador» only where a source calls it so, in the receipt |
| the value the measure is compared with | valor de referência | never «limiar» |
| the source | fonte | once per number, in the receipt link |
| the receipt page of a number | Fonte e verificação (the page's title) | «recibo» and «livro-razão» in the code and the method, not on the page |
| the project's text about a study or a place | leitura | «Em resumo», «O que este projeto conclui», «O que podia funcionar melhor» |
| the action of opening a study | Ler | never «Ler no sítio»; the original edition is «Documento original» |
| the list of coming releases | Agenda | |
| what is new | O que mudou | never «novidades», «atualizações» |
| verified on | verificado a | the date only |
| published on | publicado a | the date only |
| a study | estudo | never «trabalho», «peça», «documento» for the study itself; «documento» is the source's document |
| the housing theme | habitação | «casa» is a house and nothing else |
| home (the front page) | Portugal, or Início in the footer | never «Home» |

English words that shall not be translated one to one: site (→ projeto/página), home (→ início), dashboard (→ painel, internal only), card (→ cartão, internal only), receipt (→ the page «Fonte e verificação»), threshold (→ valor de referência), domain (→ tema), edition (→ edição, internal only), snapshot (→ a data da leitura).

## 7 · What stays as it is

The typography (the two type families, the sizes, the black rule under the name), the cards' anatomy from the P2 block (name, value, sentence, ruler, one mark), the receipts, the map and its insets, the language switch, the corrections channel, the AI label in the footer (one line), the checks.

## 8 · The blocks, in order, each on captures before landing [P]

1. **B1 · The four pages that set the grammar:** the country page (the front page), the place page (Évora as the sample, then all 308 by the same template), the study page (Évora 2027), the studies list. The menu of five. The grey cell gone. Two weeks of the three go here, because everything else follows the grammar these four fix.
2. **B2 · The themes:** the eighteen theme pages replace the domínios and the áreas de governo; the European comparison folds in.
3. **B3 · The line:** regions and districts by the same grammar; the search; the map as the door.
4. **B4 · The older studies:** their openings written by the seat and approved by Nuno; the paragraphs about the project's own method cut from their texts; the two contradictions between studies corrected.
5. **B5 · Sobre, Método, the vocabulary check and the editor's read:** Sobre and Método consolidated so that the project explains itself once; the vocabulary check in the build; and on every block from B1 on, a reader from another family whose only question is whether an editor of a Portuguese daily would print the page as it is.

## 9 · What Nuno decides [D]

1. The menu of five and its words: Portugal · Lugares · Temas · Estudos · Sobre.
2. «Temas» as the reader's word for the eighteen subjects, and the áreas de governo gone as pages.
3. The study page with one door («Ler») and the original as a line at the end.
4. The front page as the country's page, with the map as the door to the places and no line under the name.
5. The vocabulary table, corrected by him where the word is not the one he would use.
6. The four mock-ups, on the captures sent with this document.
