...
4. **Restos mortos** (§2): a consulta a `[data-sem-js]` em `inicio.js` sem elemento na página, e o atributo `data-mapa-cartao` que nada lê. Confirmado no repositório (`grep`): `[data-sem-js]` só tem elemento na régua da convergência, que está fora da primeira página; `data-mapa-cartao` só aparece num comentário. **Saem no commit de fecho deste bloco.**
