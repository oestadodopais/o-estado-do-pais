...
| pequenas-5 | 2 | por ler | as duas linhas são a MESMA frase da lei do limite de dívida que já estava declarada, relida porque o nome do diploma passou a viver num `<span lang="pt-PT">` (I91) e a régua junta os pedaços de um bloco com um espaço: o texto da página não mudou, a leitura da régua mudou. As entradas do dicionário das unidades (I92) não entram no inventário, e não é um esquecimento: a unidade rende-se dentro de um elemento com `data-linha-claim`, que é origem declarada, e as duas varreduras da régua deixam-na cair — como a `voz-3` acima, onde o texto dos registos é origem declarada e a linha fica pelo rasto |
