...
| frases | 8 | por ler | o bloco F0.9 «As sete frases de 18.08», 03.09.2026, ramo `frases-2026-09-03` (construtor Claude Opus 5); a leitura cruzada faz-se antes da fusão. Seis linhas são as três frases aparadas do painel nas duas edições (dívida pública, preços da habitação, custo unitário do trabalho) e duas são as gémeas retiradas da dívida, que não podem voltar. Duas das seis são frases que a régua NUNCA tinha visto: tirar-lhes a oração por provar tirou-lhes também a marca de origem declarada que fazia `blocosDe()` saltar o bloco inteiro, e por isso entram agora no inventário pela primeira vez. As três frases do Painel Social continuam fora da régua, num `span.social-frase` que `BLOCOS` não conhece, e é por isso que o arame do F0.9 lê o HTML construído e não a régua. O relatório do bloco está em `design/especime-v3/medicoes/frases-construtor.md` |
