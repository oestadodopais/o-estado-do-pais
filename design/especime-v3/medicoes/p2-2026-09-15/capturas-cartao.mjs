...
/**
 * ---------------------------------------------------------------------------
 * AS CAPTURAS DO BLOCO P2 · «O número e o seu sentido: o cartão de uma medida»
 * ---------------------------------------------------------------------------
 * O guião é o do F1.13 (`design/especime-v3/medicoes/porta-2026-09-15/capturas-porta.mjs`)
 ...
 *
 * O §1, item 8, do brief P2 diz o que fotografar: a página de uma área (a da
 * habitação), a página de um domínio, «Portugal na União Europeia» e a primeira
 * página, nas duas edições, a 390, 768, 1 024, 1 280 e 1 600 px, antes e depois,
 * em `design/especime-v3/capturas/p2-2026-09-15/`.
 *
 ...
 * sabe qual é.
 *
 *   node design/especime-v3/medicoes/p2-2026-09-15/capturas-cartao.mjs --momento=antes
 *   node ...../capturas-cartao.mjs --momento=depois
 *   node ...../capturas-cartao.mjs --momento=antes --so=area
 *
...
const DESTINO = path.resolve(
  RAIZ,
  PARA || path.join('design', 'especime-v3', 'capturas', 'p2-2026-09-15'),
);

/**
 * AS QUATRO PÁGINAS DO CONJUNTO, com a rota de cada edição.
 *
 * `area` é a da habitação, que é a que o diretor leu a 15.09.2026 e a que o
 * brief nomeia; `dominio` é a do primeiro domínio, que é o único com página;
 * `uniao` é «Portugal na União Europeia»; `inicio` é a primeira página. As
 * quatro rendem cartões de medida, e é o cartão que este bloco muda.
 */
const ROTAS = [
