...
Cada planta dos portões repõe os bytes e guarda os dois sha256 (9 ficheiros, 9 repostos); as saídas estão em `planta-r1-*.log`, `plantas-pais.json`, `plantas-portoes-r1.json`, `alvos-plantas-r1.json`, `cartao-prova.txt` e `plantas-motor.json`. Os 2 ficheiros brutos das corridas do `check:alvos` não entram no repositório, porque trazem o caminho da máquina; `alvos-plantas-r1.json` guarda o que deles se lê.
...
- **O evento do calendário com a marca «[verify]» no excerto**, trazido pela exportação da agenda e rendido na página da agenda (§2). Fecha-se no `indicators/calendar.json` do motor, que este bloco não toca.
...
- **O mapa do repositório**: 8 das citações dele andaram com este bloco, e `conferir-mapa.txt` diz para onde; o mapa é do lugar de direção.
