...
| chave | pt | en | nota |
|---|---|---|---|
| `inicio.portas.abrirConcelhos` · `abrirEstudos` · `abrirAgenda` | Todos os concelhos · Todos os estudos · Toda a agenda | All municipalities · All studies · The whole agenda | item 2. Cada uma das três portas passa a dizer o NOME do que está do outro lado, e não o mecanismo. **São três chaves e não uma com um buraco:** o nome de cada página entra com a preposição e o género que a sua língua lhe dá («Todos os concelhos», «Toda a agenda»), e um gabarito com um buraco escreveria «Todos os agenda». A seta compõe-se ao lado, no gabarito, como em todas as portas da casa |
| `ambito.pesquisaRotuloMapa` | Escreva o nome do concelho, ou toque no mapa. | Type the name of a municipality, or tap the map. | item 3. O rótulo da busca NA PRIMEIRA PÁGINA, que nomeia os dois caminhos para um concelho que ficam no primeiro ecrã depois de a gaveta «Os nomes no mapa» sair da vista. **É uma chave à parte e não a que já existia:** `/municipios` e `/livro-razao/concelhos` rendem a mesma caixa de busca e não têm mapa nenhum, e lá a frase prometia uma coisa que não está na página. `ambito.pesquisaRotulo` fica para essas duas, sem uma palavra mudada |
| `dominios.medidasDoisPontos` · `medidasSeparador` · `eMaisA` · `eMaisB` | `: ` · `, ` · `, e mais ` · *(vazio)* | `: ` · `, ` · `, and ` · ` more` | item 4. Pontuação, e mais nada: os nomes das medidas de cabeça vêm da declaração do domínio, com `data-nome="medidas"` e `data-de-linha`, e o numeral do «e mais N» vem de `numeralPorExtenso()`. **`eMaisB` está vazio em português de propósito:** «, e mais cinco» põe o numeral no fim e «, and five more» põe-o no meio, e um gabarito com um buraco só de um lado escrevia «and five» na edição inglesa |
| `livro.marcaAbre` · `marcaCheia` · `marcaPorConfirmar` | Ao pé de cada número, a marca da fonte: · fonte, excerto e data conferidos · um campo por confirmar. | Beside every number, the source mark: · source, excerpt and date checked · one field still to confirm. | item 5. As três cadeias são a legenda da marca da fonte, numa linha, com as duas amostras desenhadas dentro da frase. São TRÊS e não uma porque a frase tem duas amostras lá dentro; o que a régua da voz recolhe é o bloco rendido, que é a frase inteira (as amostras são `aria-hidden` e não levam texto). Substituem `livro.seloK`, `seloCheio` e `seloTracejado`, abaixo |

*E as que mudaram de texto:*

| chave | pt (antes → depois) | en (antes → depois) | nota |
|---|---|---|---|
| `identidade` | Um observatório de Portugal: cada número com a sua fonte, lido por território, por domínio e em estudos. → Os números oficiais de Portugal, do país ao seu concelho, cada um com a fonte. | An observatory of Portugal: every number with its source, read by territory, by domain and in studies. → Portugal’s official numbers, from the country to your municipality, each with its source. | item 1. A frase antiga descrevia o MÉTODO e as três maneiras de percorrer o sítio; a nova diz o que a coisa é a quem chega. A regra da Emenda 18 não muda: uma frase, por baixo da marca, na primeira página e em mais lado nenhum, sem quem a faz, sem como, sem adjetivos, sem porta e sem algarismo. **A plica inglesa é a curva (U+2019)**, que é a da edição inglesa em todo o ficheiro; o brief escreve-a direita porque é markdown, e não porque a casa mudou de tipografia. A sentinela do arame da classe, em `scripts/check-voz.mjs`, muda com ela |
| `marcador.linhaItens[0]` | o selo dessa linha desenha-se a tracejado, e não cheio; → a marca da fonte dessa linha desenha-se a tracejado, e não cheia; | the seal for that row is drawn dashed, not filled; → the source mark for that row is drawn dashed, not filled; | item 5. A marca passa a chamar-se «a marca da fonte» em toda a prosa que o leitor vê, e a página do marcador é prosa que o leitor vê. `/a-verificar` não é rota do inventário das frases (`ROTAS_DO_INVENTARIO`, em `scripts/medir-defeitos.mjs`), e por isso as duas linhas não estão lá; a L3 de `check:lugar` mede-a na mesma, porque essa régua lê todas as páginas de `dist/` |
| `marcador.voltaV` | …o selo passa a cheio e a linha volta ao índice… → …a marca passa a cheia e a linha volta ao índice… | …the seal turns solid and the row returns… → …the mark turns solid and the row returns… | item 5, a segunda das duas frases da página do marcador. Ver a linha de cima |
