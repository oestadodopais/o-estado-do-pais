...
| navegacao | Os domínios do país, e os números que este projeto já publica em cada um. | palavras-do-rodape | viva | a linha de abertura da secção dos domínios, uma vez (item 6 do brief do P1, 15.09.2026). Diz o que a lista é: os dezoito domínios da carta, e os números que já se publicam em cada um. O numeral não está no texto acima porque é contado e não escrito (`data-nonledger="numeracao"`, sobre `DOMINIOS.length`), e por isso a frase não envelhece no dia em que a carta ganhar um domínio. É `navegacao`: é a legenda de um índice. «Este projeto» e não «a casa», pela emenda do diretor das 16:35 UTC |
| navegacao | The country’s domains, and the numbers this project already publishes in each. | palavras-do-rodape | viva | a linha de abertura da secção dos domínios, uma vez (item 6 do brief do P1, 15.09.2026). Diz o que a lista é: os dezoito domínios da carta, e os números que já se publicam em cada um. O numeral não está no texto acima porque é contado e não escrito (`data-nonledger="numeracao"`, sobre `DOMINIOS.length`), e por isso a frase não envelhece no dia em que a carta ganhar um domínio. É `navegacao`: é a legenda de um índice. «Este projeto» e não «a casa», pela emenda do diretor das 16:35 UTC |
...
| conteudo | · concelhos | palavras-do-rodape | viva | a legenda de repouso do lugar do nome do mapa, no nível do país (item 5 do brief do P1, 15.09.2026), no lugar das duas frases de instrução que lá estavam. O nome do país é um lugar (`data-lugar`) e a contagem é a chave da prova `municipios_total`, com a sua porta: por isso o que sobra do bloco para a régua é a pontuação e o substantivo. É `conteudo`: diz onde o leitor está e o tamanho do que o desenho cobre |
| conteudo | · municipalities | palavras-do-rodape | viva | a legenda de repouso do lugar do nome do mapa, no nível do país (item 5 do brief do P1, 15.09.2026), no lugar das duas frases de instrução que lá estavam. O nome do país é um lugar (`data-lugar`) e a contagem é a chave da prova `municipios_total`, com a sua porta: por isso o que sobra do bloco para a régua é a pontuação e o substantivo. É `conteudo`: diz onde o leitor está e o tamanho do que o desenho cobre |
...
| conteudo | valor de referência da Comissão % · acima do valor de referência | palavras-do-rodape | viva | a linha do valor de referência dentro de uma leitura, com o rótulo que diz de quem ele é e a palavra que diz de que lado o valor está (item 8 do brief do P1, 15.09.2026). A palavra passa a dizer o LADO e não a relação com a regra, e o lado sai do sinal do limiar que a linha já declara. A gémea antiga está `retirada` |
| conteudo | Commission reference value % · above the reference value | palavras-do-rodape | viva | a linha do valor de referência dentro de uma leitura, com o rótulo que diz de quem ele é e a palavra que diz de que lado o valor está (item 8 do brief do P1, 15.09.2026). A palavra passa a dizer o LADO e não a relação com a regra, e o lado sai do sinal do limiar que a linha já declara. A gémea antiga está `retirada` |
| conteudo | valor de referência do Pacto de Estabilidade e Crescimento − % · acima do valor de referência | palavras-do-rodape | viva | a linha do valor de referência dentro de uma leitura, com o rótulo que diz de quem ele é e a palavra que diz de que lado o valor está (item 8 do brief do P1, 15.09.2026). A palavra passa a dizer o LADO e não a relação com a regra, e o lado sai do sinal do limiar que a linha já declara. A gémea antiga está `retirada` |
| conteudo | Stability and Growth Pact reference value − % · above the reference value | palavras-do-rodape | viva | a linha do valor de referência dentro de uma leitura, com o rótulo que diz de quem ele é e a palavra que diz de que lado o valor está (item 8 do brief do P1, 15.09.2026). A palavra passa a dizer o LADO e não a relação com a regra, e o lado sai do sinal do limiar que a linha já declara. A gémea antiga está `retirada` |
| conteudo | valor de referência recomendado pelo Conselho da UE % · acima do valor de referência | palavras-do-rodape | viva | a linha do valor de referência dentro de uma leitura, com o rótulo que diz de quem ele é e a palavra que diz de que lado o valor está (item 8 do brief do P1, 15.09.2026). A palavra passa a dizer o LADO e não a relação com a regra, e o lado sai do sinal do limiar que a linha já declara. A gémea antiga está `retirada` |
| conteudo | reference value recommended by the Council of the EU % · above the reference value | palavras-do-rodape | viva | a linha do valor de referência dentro de uma leitura, com o rótulo que diz de quem ele é e a palavra que diz de que lado o valor está (item 8 do brief do P1, 15.09.2026). A palavra passa a dizer o LADO e não a relação com a regra, e o lado sai do sinal do limiar que a linha já declara. A gémea antiga está `retirada` |
...
| conteudo | Portugal ultrapassa 4 valores de referência do Procedimento dos Desequilíbrios Macroeconómicos e cumpre 9 . | palavras-do-rodape | viva | a redação nova da palavra que o item 8 do brief do P1 troca (15.09.2026, pela decisão do diretor da manhã de 15.09, `DECISIONS.md` §1.108): «limiar» sai do texto que o leitor vê e fica «valor de referência». A gémea antiga está `retirada` neste ficheiro, com a mesma razão. O dono do valor continua dito: o que saiu foi a palavra, não a atribuição |
